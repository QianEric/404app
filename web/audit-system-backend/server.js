...
app.get('/api/diaries', async (req, res) => {
  ...
  try {
    ...
    const query = status && status !== 'all' ? { status, isDeleted: false } : { isDeleted: false };
...
app.put('/api/diaries/:id', async (req, res) => {
  ...
  try {
    ...
    if (status === 'deleted') updateData.isDeleted = true;

    const result = await db.collection('diary').updateOne(
      { _id: new ObjectId(id) },
      { $set: updateData }
    );
